<template>
  ...
  <swiper :spaceBetween="30" :pagination="{
    ...
    <swiper-slide v-for="element in jeux" v-bind:key="element.id">
      <img className="md:oobject-scale-down object-cover w-full h-32 lg:h-64 mt-1" v-bind:src="element.jeu_img"
        alt="" />
    </swiper-slide>
  </swiper>
  <!-- contenue -->
    ...
    backgroundImage: `url(${element.jeu_img})`,}" v-bind:key="element.id" class=" flex flex-col md:h-96 justify-between  h-60 bg-no-repeat bg-cover border-b-2 border-red-700  
      ...
      <div class="flex w-full justify-between p-2 ">
        ...
        <div class="flex space-x-2 md:h-8 h-6 p-1 justify-center">
            <button class="inline-block  md:px-2 px-1  bg-white rounded-full shadow  hover:bg-violet-600 active:bg-green-500 focus:bg-green-600 ">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class=" test  w-5 h-4 md:w-5 md:h-5 duration-300 text-red-600 hover:text-white  focus:text-white">
  ...
  backgroundColor: `${listetag.tags.couleur}`} ">{{listetag.tags.nom}}</span>
            </div>
          </div>
        </div>
      </div>
  ...
  created(){
        ...
        methods:{
            manageJeu(results){
                ...
                // this.test = this.jeux[0]['listeTags'][0]['tags'];
